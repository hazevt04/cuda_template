...
inline std::string get_cufft_status_msg( const cufftResult cufft_status) {
  if ( ( cufft_status > 0 ) && ( cufft_status < 16 ) ) {
    const std::string cufft_status_strings[] = {
    ...
    return cufft_status_strings[cufft_status];
  } else {
    return std::string{ std::string{"Unknown cufft status: "} + std::to_string(cufft_status) };
  }
}
